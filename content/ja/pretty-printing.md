...
(リストはこの先、[前章](./interfaces.md) のモノイドのスペックへと続きます — ひとつのコミットに、ふたつの章。)スイートはコンパイル時に red です:
...
これまでのスペックは — 今日のプロパティも含めて — 私たちが思いついて書き出した例を検査するものでした。[テストが定理になる](./proofs.md) は、すべての入力を一度に検査します。そしてテストランナーは、型チェッカです。
