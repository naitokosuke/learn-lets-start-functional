...
パッケージファイル `tests/tests.ipkg` は、[環境構築の章](./setup.md)で予告したとおりです。
...
最初の本物の red の時間です。[正規表現はデータである](./regex-as-data.md)では、スペックがまだ存在しない 6 つのコンストラクタを要求し、コンパイラは気持ちのいい拒絶で応えます。
