...
これが REPL サバイバルキットの全部です。式を打てば評価、`:t` で型、`:exec` でアクションの実行、`:q` で退出。[速習コース](./idris-crash-course.md)で小技がいくつか増えますが、この 4 つでかなり遠くまで行けます。
...
`build` はライブラリをコンパイルします。`test` はあなたが何百回も打つことになるコマンドです。ライブラリを再ビルドし、依存関係を更新し、テストパッケージをコンパイルして、実行します。`repl` は私たちのライブラリのモジュールを読み込める状態で REPL を開きます——エンジンを対話的につつくのに重宝します。`bench` はずっと後の[対決](./the-race.md)のためのものです。
...
次は約束していた速習コースです。この先のすべての章を読むのに十分な Idris を、[Idris 速習](./idris-crash-course.md)で。
