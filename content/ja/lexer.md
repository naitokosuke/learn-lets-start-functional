---
title: "総仕上げ:レキサ"
description: すべてが合成されて本物の道具になります — すべてのルールを 1 文字進めることが、すべてのルールを 1 文字微分することそのものになる、最長一致のレキサです。
---

# 総仕上げ:レキサ

エンジンの章は、残りひとつ。それを、上に本物を建てることに使いましょう:レキサ — あらゆるコンパイラの最初の段階 — を、画面 1 枚ほどのコードで、本書が作ってきたすべてを動力にして。

## レキサとは何か

コンパイラは `1 + 2*x` をパースする前に、テキストを**トークン**に刻みます:数の `1`、空白、プラス記号、というふうに。その裁断機がレキサ(字句解析器、トークナイザとも)で、伝統的にはルールの表 — トークン種別ごとに正規表現ひとつ — として仕様化されます。

この表を曖昧でなくする戦略は、**最長一致(maximal munch)**と呼ばれます:各位置で、どれかのルールが作れる*最長*のマッチを取る。ふたつのルールが並んだら、*先に*書いてある方が勝つ。どちらの半分も重要です。最長マッチだからこそ、`12foo` は数 `12` と `foo` の並びではなくひとつの識別子として字句解析されます — そして現実の言語で `>=` がふたつではなくひとつの演算子になるのも、同じ理由です。先勝ちのルールは、あらゆる言語がキーワードを扱う方法です:`if` はキーワードのルールにも識別子のルールにも同じ長さでマッチしますが、キーワードのルールは、表の上の方に座っているというだけの理由で勝つのです。

そして、ここが本書全体の回収の瞬間です:たくさんの正規表現を入力の上で足並みを揃えて走らせることは、バックトラック型のマッチャには本当に骨が折れます — しかし微分なら、*すべてのルールを 1 文字進めることは、すべてのルールを 1 文字微分することでしかありません*。正規表現は値。ルール表は `List`。字句解析は畳み込み。ご覧あれ。

## Red: ゲームのルール

コミット [b057007](https://github.com/ubugeeei-prod/lets-start-functional/commit/b057007416f97d6d479907844363c104ed976bf1) が [`regex/tests/src/Spec/Lex.idr`](https://github.com/ubugeeei-prod/lets-start-functional/blob/main/regex/tests/src/Spec/Lex.idr) を追加します。まず、小さな電卓言語のトークン種別 — `Eq` と `Show` は手書きですが、[正規表現はデータである](./regex-as-data.md) を経たいまでは、完全にお決まりの作業です:
...
次にルール表。4 つの章をかけて手に入れたパターン構文で書きます:
...
そしてスペック — 最長一致、タイブレーク、失敗、それから設計についての最後のひとつ:
...
最後のスペックを味わってください。たいていのレキサは「空白を読み飛ばす」フラグを生やしますが、私たちのレキサには要りません。トークンがふつうの `List` として返ってくるので、`filter` はすでに存在しているからです。出力が素のデータであれば、作るかもしれなかった機能の半分は、誰かがもう書いてくれた関数なのです。red はこちら:
...
実装のヘッダを見てください:`Show k => Show (Token k)`。実装は、*他の実装を要求*できるのです — `Token k` が印字できるのは、ちょうどその種別が印字できるとき。小さいけれど、素敵です:あらゆる関数シグネチャにあったのと同じ制約の矢印が、今度は実装の上に現れています。そしてこれこそ、リストやペアや `Maybe` の `Show` が、あなたの足元でずっと動いてきた仕組みです。
...
`step` は、正規表現が値であることの見返りをまるごと 1 行に収めたものです:ルール表全体を 1 文字進めることは、`deriv` の `map`。そして `firstAccepting` がタイブレークです:表を上から下へ走査するので、*先の*ルールが構造からして勝ちます。最長マッチの歩みはこちら:
...
最長一致を、文字どおりに:微分し続け、どれかのルールの正規表現が `nullable` になる — いままさに受理している — たびに、`best` を現在位置で上書きする。入力が尽きるか、すべてのルールが死んだら、最後に覚えた受理が最長マッチです。早期脱出にも一礼を:もう何にもマッチできなくなったルールは、たまたま充足不能なだけのだだっ広い木にではなく、*文字どおりの* `Fail` に微分されています — [スマートコンストラクタ](./smart-constructors.md) のスマートコンストラクタが、死んだ枝をその場でつぶしてくれるからです。だからこそ、素朴な `== Fail` 検査だけで、表全体が死んだことに気づけるのです。
...
二度読む価値のあるガードがふたつあります。`Just (_, Z) => Nothing` は長さゼロのマッチを拒みます:`pat "a*"` のようなルールはあらゆる位置で空文字列を受理しますし、空のトークンを無限に吐き出すレキサはレキサではありません。そして `assert_smaller` は、新しい「正直さの注釈」です:このモジュールは `%default total` ですが、全域性チェッカには、`splitAt` が作る `rest` が `cs` の真の接尾辞であることが見えません。*私たち*には見えます:`S len` というパターンが、少なくとも 1 文字は消費されたことを保証しています。`assert_smaller cs rest` は、まさにその主張にプログラマが署名する脱出ハッチです。一度だけ、コメント付きで、言葉にできる理由とともに使う — 先輩の `covering` と同じく、この注釈はコードを弱めるというより、信頼がどこから入り込むのかを正確に文書化してくれるのです。
...
> [!NOTE]
> 167 ではなく 163 です:リポジトリの履歴ではレキサはベンチマークのコミットより*前*に着地するため、この時点のスイートには [対決:線形時間 vs バックトラック](./the-race.md) の回帰スペック 4 本はまだ入っていません。コミットを順に再生すれば、同じ数字が出ます。

## 合成されるプロパティ

締めの一振りです。このレキサは決してバックトラックしません — 私たちが注意深かったからではなく、*できない*からです:エンジンの操作は `deriv` と `nullable` だけ、各ルールは入力 1 文字につきちょうど 1 回進み、`tokenize` は消費済みのテキストを二度と訪れません。長さ n の文字列を m 本のルールで字句解析するコストは、ルール 1 本あたり n 回の微分ステップ。それだけです。ルールが何であっても。エンジンに作り込んだ線形時間の性質は、その上に建てられても生き延びた — どころではありません。*合成*されたのです。それが本書全体の静かなテーゼです:芯を正しく作り、すべてをデータと関数にすれば、良い性質はタダで上へと旅をする。

## まとめ

- レキサは、ルール表 — トークン種別ごとに正規表現ひとつ — を使い、最長一致のもとでテキストをトークンにします:最長マッチが勝ち、引き分けは先のルールへ(これがキーワードが識別子に勝つ仕組みです)。
- 正規表現が値なら、ルール表は `List (k, Regex)` で、全ルールを進めるのは `map (deriv c)` — 1 行です。
...
- レキサはエンジンから線形性を相続します:どのルールも決してバックトラックしないので、この総仕上げは、いつでも一方通行の 1 パスで走ります。

エンジンは完成し、競走し、証明され、仕事に就きました。残るのは、自分が作り上げたものを眺めること — そして、それをどこへ持っていくかです。[この先へ](./whats-next.md) へ。
