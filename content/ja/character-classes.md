...
[前の章](./smart-constructors.md) のスマートコンストラクタと同じ一手です。かつてコンストラクタだったものが、一般的な形を組み立てる小文字の関数になりました。AST は精神においては*小さく*なりました — コンストラクタは相変わらず 6 つですが、そのうちのひとつが、リテラルもクラスもエスケープもワイルドカードも、まとめて面倒を見るようになったのです。
...
次は `+`、`?`、`{n,m}` と文字列リテラルを追加します — そして、それらが何のコストもかからないことを発見します。[糖衣構文はただの関数](./sugar.md) へ。
