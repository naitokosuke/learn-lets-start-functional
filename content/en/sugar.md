...
And the star of the file: a real pattern, assembled from everything this chapter and the [last one](./character-classes.md) built. A `where` block keeps it readable:
...
Each definition is its own specification read aloud. Note they build with the smart constructors `cat`, `alt`, `star` — sugar gets the simplification algebra from [Smart Constructors](./smart-constructors.md) for free.
...
The date pattern was still painful to write as function calls — next we build the tool that will let users write `\d{4}-\d{2}-\d{2}` directly, starting with [Parser Combinators](./parser-combinators.md).
