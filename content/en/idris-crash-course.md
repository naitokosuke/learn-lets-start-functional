...
You have Idris installed from [the last chapter](./setup.md); now you learn to speak it. This is not a language reference — it is the minimum Idris needed to read every chapter that follows, learned the way the rest of the book works: by typing things and watching what comes back.
...
- `do` sequences `IO` actions and for now just means "then"; the deeper story waits until [Parser Combinators](./parser-combinators.md).

Now you can read Idris — time to write some, starting with the tool every later chapter stands on: [TDD and a Tiny Test Harness](./tdd.md).
