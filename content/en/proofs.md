...
A failing *compile* has been a perfectly good red since [TDD and a Tiny Test Harness](./tdd.md). This time it is the whole show.
...
Read the type as a sentence: *for every regex `r`, if `nullable r = True`, then there is evidence that `r` matches `[]`*. The function body is the proof, and "induction on `r`" turns out to be technology you already own — pattern matching on each constructor, recursive calls for subterms. The `Fail` case: the premise claims `nullable Fail = True`, i.e. `False = True` — `absurd`. The `Cat` case: split the `&&` with `andBoth`, recurse on both halves, and note quietly that `MCat` here matches `[] ++ []`, which the compiler *computes* to `[]` — no lemma needed. Because the totality checker accepts this function, the induction is well-founded; a partial function would be a circular argument, and Idris would refuse it.
...
The lovely detail: `MSym` demands evidence that `member c s = True`, and the evidence is just `Refl` — because `member 'a' (single 'a')` is a closed expression the type checker *runs*, and it computes to `True`. Our ordinary, executable `member` function from [Character Classes](./character-classes.md) is doing double duty as part of the logic. Programs and proofs are not two worlds here; the whole point of dependent types is that they are one.
...
Theory has had its say; next we go racing. [The Race: Linear vs Backtracking](./the-race.md) finally builds the rival engine this book has been warning you about — and the story takes a turn nobody scripted.
