...
You have been using interfaces since [A Regex Is Data](./06-regex-as-data.md) without ever stopping to name them. This chapter finally names them, and then runs straight into a question that Haskell answers with a workaround and Idris answers with a feature.

## You already use interfaces

Back in [A Regex Is Data](./06-regex-as-data.md), we wrote this and moved on:
...
And the test harness from [TDD and a Tiny Test Harness](./05-tdd.md) has carried this signature the whole way:

```idris
shouldBe : Show a => Eq a => String -> (actual : a) -> (expected : a) -> Spec
```

In [Parser Combinators](./13-parser-combinators.md) we climbed the ladder from `Functor` to `Alternative`. So the machinery is old news. What we have never done is say precisely what the three moving parts are:
...
- An **implementation** is the fulfilment of that contract for one specific type. Here is the part worth internalizing: an implementation is a *value*, a record of functions that the compiler builds once and then passes around on your behalf. The `Show Regex where ...` block above constructs that value.
...
And numbers, twice over. Addition is associative and `0` changes nothing; multiplication is associative and `1` changes nothing. Both are perfectly good monoids on the same type.
...
We built both facts into the smart constructors back in [Smart Constructors](./10-smart-constructors.md): `cat Eps r = r` and `alt Fail r = r` are the identity laws, written as code. Time to make the algebra official, starting with a spec.
...
- `[SeqSemigroup] Semigroup Regex where ...` is an ordinary implementation with a name in front. Because it is named, it does not become the default, and it coexists peacefully with `AltSemigroup` on the same type.
...
> [!NOTE]
> This commit pair delivers more than the monoids: the same spec file opens with tests for `toPattern`, a pretty-printer that turns a `Regex` back into pattern syntax, and the green commit implements it. That is a story of its own, and it gets the [next chapter](./17-pretty-printing.md).
...
But notice what kind of sentence a law is: "for **all** `a`, `b`, `c`, ...". Our tests can only ever check finitely many examples of it. Idris can do better: it can state a for-all sentence as a *type* and check a proof of it at compile time. That is not a rhetorical flourish; it is [two chapters away](./18-proofs.md).
...
- Monoid laws are contracts the compiler does not check here, but Idris could, and in [Tests Become Theorems](./18-proofs.md) it will.

The same green commit also taught the engine to print its patterns back out; [Printing Patterns Back](./17-pretty-printing.md) tells that story.
