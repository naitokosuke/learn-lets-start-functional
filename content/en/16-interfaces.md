...
You have been using interfaces since [A Regex Is Data](./06-regex-as-data.md) without ever stopping to name them. This chapter names them — and then runs straight into a question that Haskell answers with a workaround and Idris answers with a feature.
...
We built both facts into the smart constructors back in [Smart Constructors](./10-smart-constructors.md): `cat Eps r = r` and `alt Fail r = r` are the identity laws, written as code. Time to make the algebra official — and to write it as a spec first.
...
> [!NOTE]
> This commit pair delivers more than the monoids: the same spec file opens with tests for `toPattern`, a pretty-printer that turns a `Regex` back into pattern syntax, and the green commit implements it. That is a story of its own — it gets the [next chapter](./17-pretty-printing.md).
