...
The [previous chapter](./why-functional.md) promised a villain and a forgotten idea that defeats it. This chapter delivers both, and ends with the roadmap for the rest of the book.
...
This view — a pattern is a *description*, and matching is checking membership in what it describes — sounds like a philosophical nicety. It is actually the engineering heart of this book. A description is just data. And in a functional language, functions over data structures are what we do all day. Hold that thought.
...
Keeping the per-step cost small does take actual work — that is what the [Smart Constructors](./smart-constructors.md) chapter is about — and we will measure the whole promise against a real backtracker, on this exact pattern family, in [The Race](./the-race.md).
...
Each step consumes one character and yields a new pattern; the old pattern and the old character are never consulted again. When the input runs out, one final question is asked of whatever pattern remains: "would you accept the empty string?" That final question is a function called `nullable`, and it is so central it gets [its own chapter](./nullable.md).
...
**Introduction** — you are here — gets your machine [set up](./setup.md), teaches you [enough Idris to read the rest](./idris-crash-course.md), and builds the [tiny test harness](./tdd.md) that every subsequent chapter leans on.

**The Core Engine** is the heart of the book: patterns as a [data type](./regex-as-data.md), the [nullable](./nullable.md) test, the [derivative](./derivatives.md) itself, and finally [matches](./matches.md) — a complete, correct engine in remarkably few lines.

**Making It Practical** turns correct into usable: [smart constructors](./smart-constructors.md) to keep derivatives small, [character classes](./character-classes.md), the familiar [sugar](./sugar.md) like `+` and `{n,m}`, a [parser-combinator library built from scratch](./parser-combinators.md), a [parser for real pattern syntax](./pattern-syntax.md), and a clean [public API](./public-api.md).

**Idris Power-Ups** is the payoff for choosing this language: [interfaces and two monoids hiding in our engine](./interfaces.md), [printing patterns back out](./pretty-printing.md), a chapter where [a test becomes a machine-checked theorem](./proofs.md), the [head-to-head race](./the-race.md) against a backtracker we build ourselves, a [working lexer](./lexer.md) as capstone, and [where to go next](./whats-next.md).
...
- Catastrophic backtracking is combinatorial re-exploration of choice points: `(a?){3}a{3}` on `"aaa"` already explores 8 combinations, and `(a?){n}a{n}` explores 2^n — the mechanism behind ReDoS attacks and the 2016 Stack Overflow and 2019 Cloudflare outages.
...
Before any of that theory can become code, you need Idris 2 on your machine — that is [Setting Up](./setup.md).
