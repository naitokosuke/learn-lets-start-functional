...
The application syntax is `double 21`, no parentheses around arguments. Multi-argument functions just stack arrows. Here is the standard library's addition on `Nat`:
...
`MkPoint 3 4` builds one, and fields come out with dot syntax, `p.x`, just like the languages you already know. Local names are introduced with `let ... in`:
