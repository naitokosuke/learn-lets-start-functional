...
The same move as the smart constructors in the [previous chapter](./smart-constructors.md): what used to be a constructor is now a lowercase function that builds the general shape. The AST got *smaller* in spirit — still six constructors, but one of them now covers literals, classes, escapes and the wildcard alike.
...
Next, we add `+`, `?`, `{n,m}` and string literals — and discover they cost nothing, in [Sugar Is Just Functions](./sugar.md).
