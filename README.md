...
Learn functional programming by building a linear-time regular expression engine in [Idris 2](https://www.idris-lang.org/), test-first.

This repository contains:

- **`regex/`**: the engine itself, written in Idris 2 with TDD. Matching runs in a single left-to-right pass over the input, so there is no backtracking to blow up.
- **`content/`**: the tutorial book (English / 日本語), built with [Vite+](https://voidzero.dev/) and [Ox Content](https://ox-content.void.app/).

## Why?

Functional programming has a reputation for being hard to start. This tutorial takes the opposite route: build one small, real, useful thing (a regex engine) and let each language feature earn its keep, one chapter at a time.
...
| # | English | 日本語 |
|---|---------|--------|
| 01 | [Why Functional? Why Idris?](content/en/01-why-functional.md) | [なぜ関数型?なぜIdris?](content/ja/01-why-functional.md) |
...
| 04 | [An Idris Crash Course](content/en/04-idris-crash-course.md) | [Idris速習](content/ja/04-idris-crash-course.md) |
| 05 | [TDD and a Tiny Test Harness](content/en/05-tdd.md) | [TDDと小さなテストハーネス](content/ja/05-tdd.md) |
...
| 15 | [A Public API](content/en/15-public-api.md) | [公開API](content/ja/15-public-api.md) |
...
| 19 | [The Race: Linear vs Backtracking](content/en/19-the-race.md) | [対決:線形時間vsバックトラック](content/ja/19-the-race.md) |
...
The book deploys to [Void](https://void.cloud/) from GitHub Actions using OIDC, so there is no long-lived secret to store. One-time setup:
